...
function PublicHome({ summary, trend, summaryLoading, trendLoading, summaryError, trendError }: HomeDataProps) {
  return (
    <main className="page public-home">
      ...
      <section className="public-intelligence-section">
        <div className="section-intro-row">
          <div><p className="eyebrow">Four ways to read the network</p><h2>One flight record. Four useful questions.</h2></div>
          <p>The public brief shows the answer in everyday language. The researcher workspace keeps the evidence and controls behind it.</p>
        </div>
        <div className="public-intelligence-grid">
          <PublicIntelligenceCard index="01" label="Performance" title="What happened?" copy="Compare airlines, airports, and routes using the historical operating record." href="/carriers" accent="teal" action="Explore the record" />
          <PublicIntelligenceCard index="02" label="Capacity" title="How full was the system?" copy="Put passengers and available seats beside the route outcome, without pretending association proves cause." href="/capacity" accent="violet" action="See the context" />
          <PublicIntelligenceCard index="03" label="Schedule" title="What could be tested?" copy="Explore the idea of moving a small number of flights away from the busiest period." href="/decision-center" accent="amber" action="See the experiment" />
          <PublicIntelligenceCard index="04" label="Network" title="What connects the system?" copy="Separate the busiest airports from the airports that structurally connect different parts of the network." href="/decision-center" accent="blue" action="See the network view" />
        </div>
      </section>
...
function ResearchHome({ summary, trend, summaryLoading, trendLoading, summaryError, trendError }: HomeDataProps) {
  return (
    <main className="page research-home">
      ...
      <section className="research-intelligence-section"><div className="section-intro-row"><div><p className="eyebrow">Four analytical lanes</p><h2>Choose the kind of intelligence you need.</h2></div><span className="section-note">Every lane ends in a visible decision question</span></div><div className="research-lane-grid"><ResearchLane index="01" label="Performance explorer" title="What happened, and where?" copy="Measure historical reliability by carrier, airport, route, time, and delay category." href="/carriers" accent="teal" /><ResearchLane index="02" label="Capacity + demand" title="Does traffic context move with reliability?" copy="Join T-100 passengers, seats, and operated flights to the same carrier-route-month as OTP." href="/capacity" accent="violet" /><ResearchLane index="03" label="Schedule experiment" title="What bounded change could reduce pressure?" copy="Test small shifts inside a departure bank and compare the observed peak before and after." href="/decision-center" accent="amber" /><ResearchLane index="04" label="Network resilience" title="Which airports connect the system?" copy="Compare raw volume with structural connector scores to find hubs and bridges." href="/decision-center" accent="blue" /></div></section>
      <section className="research-surface"><div className="section-intro-row"><div><p className="eyebrow">Research index</p><h2>Supporting surfaces for the investigation.</h2></div><span className="section-note">Choose a surface, then narrow the question</span></div><div className="research-module-grid"><ResearchModule href="/carriers" index="01" label="Carriers" title="Who is performing differently?" /><ResearchModule href="/airports" index="02" label="Airports" title="Where is pressure concentrated?" /><ResearchModule href="/routes" index="03" label="Routes" title="Which connections carry the signal?" /><ResearchModule href="/delays" index="04" label="Delay causes" title="What categories are recorded?" /><ResearchModule href="/aircraft" index="05" label="Aircraft" title="How does equipment shape operations?" /><ResearchModule href="/capacity" index="06" label="T-100 capacity" title="What supply was scheduled?" /><ResearchModule href="/decision-center" index="07" label="Decision Center" title="What should I investigate next?" featured /><ResearchModule href="/compare" index="08" label="Compare" title="Is the difference meaningful?" /><ResearchModule href="/data-health" index="09" label="Data health" title="Can I trust the coverage?" /></div></section>
...
function PublicIntelligenceCard({ index, label, title, copy, href, accent, action }: { index: string; label: string; title: string; copy: string; href: string; accent: string; action: string }) { return <Link href={href} className={`public-intelligence-card public-intelligence-${accent}`}><span className="public-intelligence-index">{index}</span><span className="public-intelligence-label">{label}</span><h3>{title}</h3><p>{copy}</p><span className="public-intelligence-action">{action} <b>↗</b></span></Link>; }
...
function ResearchLane({ index, label, title, copy, href, accent }: { index: string; label: string; title: string; copy: string; href: string; accent: string }) { return <Link href={href} className={`research-lane research-lane-${accent}`}><span className="research-lane-index">{index}</span><span className="research-lane-label">{label}</span><h3>{title}</h3><p>{copy}</p><span className="research-lane-arrow">Open lane →</span></Link>; }
